    ...
    private void createChunk(Dispatcher dispatcher, Connection connection, Coordinates position) {
        ...
        dispatcher.onReserveEntityIdsResponse(op -> {
            if (op.requestId.equals(entityIdReservationRequestId) && op.statusCode == StatusCode.SUCCESS) {
                ...
                builder.addComponent(ChunkStorage.COMPONENT, new ChunkStorageData(tmpChunk), CommonWorkerRequirements.getAllCommonWorkers());
                builder.addComponent(Position.COMPONENT, new improbable.PositionData(position), CommonWorkerRequirements.getAllCommonWorkers());
